...
impl UsersDaTa {
    ...
    pub async fn update_user(
        ...
        let user_event = user_events::ActiveModel {
            ...
            details: ActiveValue::Set(Some(format!(
                "User {updated_user_id} updated by existing user {updated_user_id}"
            ))),
            creation_date: ActiveValue::Set(Some(NaiveDateTime::new(current_date.date_naive(), current_date.time()))),
            ..Default::default()
        };
    ...
    // pub async fn delete_single_user(user_model: UserModel) -> Result<u64, CodeMessage> {
    ...
    //             "User id {deleted_user_id} deleted by existing user id {deleted_user_id}"
    //         )),
