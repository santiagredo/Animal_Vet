...
impl AppointmentsData {
    pub async fn insert_appointment(
        ...
        let appointment = ActiveModel {
            ...
            is_canceled: ActiveValue::Set(appointments_model.is_canceled),
    ...
    pub async fn select_appointments(
        db: &DatabaseConnection,
        appointments_model: AppointmentsModel,
        opt_appointments_model: Option<AppointmentsModel>,
        ...
        if appointments_model.date.is_some() && opt_appointments_model.is_none() {
            condition = condition.add(Column::Date.eq(appointments_model.date));
        }

        if appointments_model.date.is_some()
            && opt_appointments_model
                .as_ref()
                .is_some_and(|x| x.date.is_some())
        {
            condition = condition.add(Column::Date.gte(appointments_model.date));
            condition = condition.add(Column::Date.lte(opt_appointments_model.unwrap().date));
        }
        ...
        if condition.len() > 0 {
            match Appointments::find().filter(condition).all(db).await {
                Err(err) => {
                    error_span!("error - database", error = ?err);
                    return Outcome::Error(CodeMessage {
                        http_code: http::StatusCode::INTERNAL_SERVER_ERROR,
